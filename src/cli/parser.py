...
def build_parser() -> argparse.ArgumentParser:
    ...
    analysis.add_argument(
        "--tingwu-enabled",
        action="store_true",
        help="Enable post-class Tingwu processing pipeline (audio recording + async worker)",
    )
    analysis.add_argument(
        "--tingwu-poll-interval-sec",
        type=float,
        default=30.0,
        help="Polling interval seconds for Tingwu task status",
    )
    analysis.add_argument(
        "--tingwu-max-wait-hours",
        type=float,
        default=6.0,
        help="Maximum wait hours for Tingwu task completion",
    )
    ...
    tingwu_process = subparsers.add_parser(
        "tingwu-process",
        help="Run async Tingwu post-processing worker from a generated job file",
    )
    tingwu_process.add_argument(
        "--job-file",
        required=True,
        help="Path to Tingwu job JSON file generated by analysis mode",
    )
    tingwu_process.add_argument(
        "--resume-task-id",
        default="",
        help="Resume an existing Tingwu task before creating a new one",
    )
